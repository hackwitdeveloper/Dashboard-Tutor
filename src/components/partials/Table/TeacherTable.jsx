...
const TeacherTable = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://52.206.149.246:3000/teacherget');
    ...
    const handleDelete = async (tutor_id) => {
        try {
            const response = await axios.delete(`http://52.206.149.246:3000/teacherDelete?tutor_id=${tutor_id}`);
            ...
            <table className="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                <thead className="bg-slate-200 dark:bg-slate-700">
                    <tr>
                        ...
                        <th className=" table-th " >EXPIERIENCE</th>
                ...
                <tbody className="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                    {page.map((row) => {
                        prepareRow(row);
                        return (
                            <tr {...row.getRowProps()} key={row.original.tutor_id}>
                                ...
                                <td className="table-td">
                                    <div className="d-flex justify-around rtl-space-x-reverse">
                                        <Tooltip content="View Details" placement="top" arrow animation="shift-away">
                                            <Link to={`/tutor?tutor_id=${row.original.tutor_id}`}  className="action-btn">
                                        ...
                                        >
                                            <button
                                                ...
                                                type="button"
                                                onClick={() => handleDelete(row.original.tutor_id)}
                                            >
